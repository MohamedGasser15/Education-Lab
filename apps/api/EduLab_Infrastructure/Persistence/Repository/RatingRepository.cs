...
namespace EduLab_Infrastructure.Persistence.Repositories
{
    ...
    public class RatingRepository : Repository<Rating>, IRatingRepository
    {
        ...
        /// <summary>
        /// Retrieves rating summary raw data for a specific course (average, total, distribution)
        /// </summary>
        /// <param name="courseId">Course identifier</param>
        /// <param name="cancellationToken">Cancellation token to cancel the operation</param>
        /// <returns>Tuple containing AverageRating, TotalRatings, and RatingDistribution</returns>
        public async Task<(double AverageRating, int TotalRatings, Dictionary<int, int> RatingDistribution)> GetCourseRatingSummaryRawAsync(
            int courseId,
            CancellationToken cancellationToken = default)
        {
            const string operationName = "GetCourseRatingSummaryRawAsync";
            ...
            try
            {
                ...
                if (!ratings.Any())
                {
                    ...
                    // Return default distribution (all zeros for stars 1-5)
                    var emptyDistribution = Enumerable.Range(1, 5)
                        .ToDictionary(i => i, i => 0);

                    return (0, 0, emptyDistribution);
                }
                ...
                var distribution = Enumerable.Range(1, 5)
                    .ToDictionary(i => i, i => ratings.Count(v => v == i));

                _logger.LogInformation(
                    "Successfully generated raw rating summary for Course: {CourseId}. Average: {AverageRating}, Total: {TotalRatings}",
                    courseId, average, ratings.Count);

                return (Math.Round(average, 1), ratings.Count, distribution);
            }
            catch (OperationCanceledException)
            {
                _logger.LogWarning("Operation {OperationName} was cancelled for Course: {CourseId}",
