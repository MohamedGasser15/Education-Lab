...
class _CertificateViewScreenState extends State<CertificateViewScreen> {
  ...
  CertificateModel _getActiveCertificate(List<CertificateModel> certificates) {
    ...
    return CertificateModel(
      ...
      verifyUrl: ApiConstants.fullUrl(
        ApiConstants.verifyCertificatePath(widget.certificateCode),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.initialCertificate != null &&
        !certList.any(
          (c) =>
              c.certificateCode == widget.initialCertificate!.certificateCode,
        )) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: isLoading && certList.isEmpty
            ...
            : (certList.isEmpty &&
                  widget.initialCertificate == null &&
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                context.loc.certVerifiedFullRequirements(
                      ...
                      child: AspectRatio(
                        ...
                        child: CustomPaint(
                          painter: _CertificateSvgPainter(
                            ...
                            certCompletionTitle:
                                context.loc.certCompletionTitle,
                            certCompletionSubtitle:
                                context.loc.certCompletionSubtitle,
                            certAnnounceStudent:
                                context.loc.certAnnounceStudent,
                            certCompletionRequirementsMet:
                                context.loc.certCompletionRequirementsMet,
                            certIssueDateText: context.loc.certIssueDateText(
                              activeCert.formattedDate,
                            ),
                            certIdNumberText: context.loc.certIdNumberText(
                              activeCert.certificateCode,
                            ),
                            certPlatformManagement:
                                context.loc.certPlatformManagement,
                            certInstructorRoleTitle:
                                context.loc.certInstructorRoleTitle,
                          ),
                        ),
                      ),
                    ),
                    ...
                    children: [
                      ...
                      Expanded(
                        child: AppButton(
                          ...
                          onPressed: () =>
                              _downloadCertificate(activeCert, 'PDF'),
                        ),
                      ),
                      ...
                      Expanded(
                        child: AppButton(
                          ...
                          onPressed: () =>
                              _downloadCertificate(activeCert, 'PNG'),
                        ),
                      ),
                    ],
                  ),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        _buildMetaRow(
                          context.loc.certStudentNameLabel,
                          activeCert.studentName,
                          textColor,
                          textSubColor,
                        ),
                        _buildMetaRow(
                          context.loc.certCourseLabel,
                          activeCert.courseTitle,
                          textColor,
                          textSubColor,
                        ),
                        _buildMetaRow(
                          context.loc.certInstructorLabel,
                          widget.instructorName,
                          textColor,
                          textSubColor,
                        ),
                        _buildMetaRow(
                          context.loc.certIssueDateLabel,
                          activeCert.formattedDate,
                          textColor,
                          textSubColor,
                        ),
                        _buildMetaRow(
                          context.loc.certCodeLabel,
                          activeCert.certificateCode,
                          textColor,
                          textSubColor,
                          isCode: true,
                        ...
                        _buildMetaRow(
                          context.loc.certIssuerLabel,
                          context.loc.certIssuerName,
                          textColor,
                          textSubColor,
                        ),
                      ],
...
class _CertificateSvgPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(
      poly1,
      Paint()..color = const Color(0xFF0A1628).withValues(alpha: 0.04),
    );
    ...
    canvas.drawPath(
      poly2,
      Paint()..color = const Color(0xFF2563EB).withValues(alpha: 0.08),
    );
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(35, 35, 1344, 930),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(47, 47, 1320, 906),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(53, 53, 1308, 894),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(667, 80, 80, 80),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(665, 78, 84, 84),
    ...
    canvas.drawCircle(
      const Offset(707, 195),
      3,
      Paint()..color = const Color(0xFF2563EB).withValues(alpha: 0.5),
    );
    ...
    final nameLinePaint = Paint()
      ...
      ..strokeWidth = 2.0;
    canvas.drawLine(
      const Offset(457, 445),
      const Offset(957, 445),
      nameLinePaint,
    );
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        const Rect.fromLTWH(647, 720, 120, 120),
  ...
  void _drawQrGraphic(Canvas canvas, Rect rect) {
    ...
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 6, rect.top + 6, 26, 26),
      qrPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 10, rect.top + 10, 18, 18),
      Paint()..color = Colors.white,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 13, rect.top + 13, 12, 12),
      qrPaint,
    );

    canvas.drawRect(
      Rect.fromLTWH(rect.right - 32, rect.top + 6, 26, 26),
      qrPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.right - 28, rect.top + 10, 18, 18),
      Paint()..color = Colors.white,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.right - 25, rect.top + 13, 12, 12),
      qrPaint,
    );

    canvas.drawRect(
      Rect.fromLTWH(rect.left + 6, rect.bottom - 32, 26, 26),
      qrPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 10, rect.bottom - 28, 18, 18),
      Paint()..color = Colors.white,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 13, rect.bottom - 25, 12, 12),
      qrPaint,
    );

    canvas.drawRect(
      Rect.fromLTWH(rect.left + 42, rect.top + 12, 16, 6),
      qrPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 40, rect.top + 26, 8, 8),
      qrPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 54, rect.top + 38, 12, 12),
      Paint()..color = const Color(0xFF2563EB),
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 22, rect.top + 46, 12, 8),
      qrPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 72, rect.top + 50, 14, 8),
      qrPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.left + 42, rect.bottom - 28, 18, 8),
      qrPaint,
    );
    canvas.drawRect(
      Rect.fromLTWH(rect.right - 28, rect.bottom - 26, 14, 14),
      qrPaint,
    );
  }
